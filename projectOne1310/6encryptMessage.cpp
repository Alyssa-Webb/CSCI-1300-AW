...
{
    int temp = 0; // creating temp variable to store integers from method 1 & 2 & 3
    ...
    switch (method_choice)
    {
        ...
        default: 
            cout << "Invalid encryption method" << endl;
            return encrypted_message; // returns unshifted message 
            break;
    }
    return encrypted_message;
}
...
assert (encryptMessage ("hello world!", 1, "unused", "unused") == "ifmmp xpsme!");
assert (encryptMessage("its a message to encrypt z!", 3, "vayyyk", "key") == "mxw e qiwweki xs irgvctx d!");
assert (encryptMessage ("ab", 2, "eeeeeeeeeeeeehhh", "unused") == "de");
assert (encryptMessage ("aaa aaa aaa aaa", 4, "first key", "other key") == "bmh mhb hbm bmh");

}
// string message, int method_choice, string key1, string key2